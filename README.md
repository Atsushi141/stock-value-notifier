...
4. 実行オプションを設定：
   - **Screening mode**: `curated`（130銘柄）、`all`（全銘柄）、または`rotation`（ローテーション）
   - **Target date**: 分析対象日（YYYY-MM-DD形式、直近7日以内、空欄で当日）
   - **Force execution**: 市場休場日でも強制実行するかどうか
5. `Run workflow` を再度クリックして実行

#### 日付指定実行

市場がクローズしている時でも、直近7日間のいずれかの日付を指定して実行できます：

**使用例**:
- **平日の過去データ分析**: `2025-01-08`（水曜日のデータで分析）
- **週末の振り返り分析**: `2025-01-10`（金曜日のデータで分析）+ `Force execution: true`
- **祝日の補完実行**: 祝日に実行できなかった分を後日実行

**制限事項**:
- 指定可能な日付は直近7日以内のみ
- 未来の日付は指定不可
- 日付形式は`YYYY-MM-DD`（例：2025-01-10）

**強制実行オプション**:
- `Force execution: true`を設定すると、市場休場日（土日・祝日）でも実行可能
- 過去の営業日データを使用した分析が可能
- 通知メッセージに指定日付が表示される
